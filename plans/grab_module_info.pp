plan mod_upgrade_check::grab_module_info (
  ...
  $puppet_master_directory = run_task('mod_upgrade_check::get_current_directory', $nodes)
  ...
  $code_environments_array.each |String $current_code_environment| {
    ...
    run_task('mod_upgrade_check::mkdir_p_file',$nodes, path => $target_directory, content => $modules_split.message)
